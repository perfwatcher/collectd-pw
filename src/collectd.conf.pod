...
=head2 Plugin C<jsonrpc>

The I<Jsonrpc> plugin makes it possible to communicate through a HTTP connection
with JSON-RPC protocol.
...
Synopsis:
 ...
 <Plugin "jsonrpc">
   ...
   Authfile "/etc/collectd.passwd.txt"
   JsonrpcCacheExpirationTime 60
   TopPsDataDir "/var/lib/collectd/top"
   RRDToolPath "/usr/bin/rrdtool"
   RRDCachedDaemonAddress "unix:/var/run/rrdcached.sock"
 </Plugin>
...
=item B<BindAddress> I<Argument>

Argument is the IP address the JSON-RPC HTTP server listens on. If unset or if set to "*", it will
listen on 0.0.0.0.

=item B<Port> I<Argument>

Argument is the port number the JSON-RPC HTTP server listens on.

=item B<ConnectionTimeout> I<Argument>

Argument is the connection timeout.

=item B<Authentication> I<Argument>

Argument is one of I<None> or I<Basic>. If using I<None>, there will be no
authentication. Everyone can connect to jsonrpc. If using I<Basic>, the
authentication will be of type Basic as defined in RFC2617 and you will be
prompted a login and a password.

=item B<Authfile> I<Argument>

When using Basic authentication scheme, Argument is a filename where to store
login/password credentials. Each line of this file has the following syntax :

  login password

Example :
  Joe     $6$Joe_J03D$EwsXtIFSSCSB7Q36NYCCBjqjXs1Yna3ruVEj/b57t2Oq7g1gbwnHbREefsRpG6ki0dIl94bFuB3pT8TCkGYCe1
  Jack    $1$J.D4lt0n$.uCooL.00nv/UphluGAe1.
  William wdRHh/Alm41TQ
  Avrael "rantanplan"

Use I<#> for comments.
Spaces can be either ' ' or '\t'.

The password can have 3 possible forms :

=over 4

=item "clear password"

The password is stored in clear form. Please protect your file ! Notice
the double quotes that is part of the syntax.

=item cryptedpassword

The password is stored crypted as in crypt(3). The 2 first chars is the
salt and the following is the crypted password. Generate a password with
tools like htpasswd (from apache) or like this :

  perl -e 'print crypt("Passw0rd", "s0")."\n";'

Where I<Passw0rd> is the clear text password and I<s0> is a salt (salt is 2 chars).

=item $id$salt$cryptedpassword

The password is stored crypted as in crypt(3). The dollars are part of the
syntax. I<id> is the crypt algorithm to use (for example, 1 means md5). I<salt>
is a up to 8 chars salt. I<cryptedpassword> is the crypted password.
Generate a password like this :

  perl -e 'print crypt("Passw0rd", "\$1\$SaLts4lT")."\n";'

where Passw0rd is the clear text password and SaLts4lT is a salt (salt is up to 8 chars).

See B<man crypt> for more information.

=back

WARNINGS :
If your jsonrpc module was not compiled with libcrypt, only clear text password
will work.

If your system does not have crypt_r(3), it will use crypt(3) with a mutex (which may
slow the performances of the server). If you do not have crypt_r(3) and feel your server
too slow, try to use clear text passwords only.

=item B<MaxClients> I<Argument>

Argument is the maximum number of simultaneous connexions.

=item B<JsonrpcCacheExpirationTime> I<time>

The jsonrpc plugin has a local cache of the types. It is a copy of the collectd cache.
Because using the collectd cache would cost a lot of time to collectd, jsonrpc has its
own cache.

Time (in seconds) specifies how long to keep the cache and when to refresh it from the
collectd cache.

=item B<TopPsDataDir> I<path>

If the plugin was compiled with support for TopPs, this is the path where to find
files from the write_top plugin.

=item B<RRDToolPath> I<path>

Path of rrdtool.

=item B<RRDCachedDaemonAddress> I<path>

Path of the rrdcached communication socket.
...
=item B<LogNotification> B<true>|B<false>

When disabled, C<logfile> do not log notification. Defaults to B<true>.
...
This plugin collects statistics about NFS (client and server).

The configuration options can be used to have a specific configuration
of what to collect from /proc/self/mountstat.

Example :

 <Plugin "nfs">
   EnableClientStatsPerMountpoint <true|false>
   <Mountpoint "all">
     MinAge 3600
     Enable true
   </Mountpoint>
   ...
   <Mountpoint "/mynfsmountpoints/data1">
     MinAge 0
     Enable true
   </Mountpoint>
   <Mountpoint "/mynfsmountpoints/data2">
     MinAge 0
     Enable true
   </Mountpoint>
 </Plugin>

The following options are accepted by the C<nfs plugin>:

=over 4

=item B<EnableClientStatsPerMountpoint> I<boolean>

Enable collecting statistics from /proc/self/mountstats (if available).

=item B<Mountpoint> I<mountpoint>

This starts a block with options for I<mountpoint>. If I<mountpoint> is "all",
the options are defined for all mountpoints unless redefined with other B<mountpoint>
blocks.

=over 4

=item B<Enable> B<true|false>

Enable or not this mountpoint.

=item B<MinAge> I<seconds>

Ignore mountpoint unless its age is more than I<seconds>. Do not ignore if set to 0.
...
=head2 Plugin C<notify_file>

This plugin writes a notification to the disk.

=over 4

=item B<DataDir> I<path>

=item B<Plugin> I<type>

=item B<InvertPluginList> B<true>|B<false>

=item B<LinkLast> I<type>

=back

Write notifications to I<DataDir>/I<hostname>/I<plugin>[-I<instance>]/I<timestamp path>/I<type>[-I<instance>]-I<timestamp>.gz

I<timestamp path> is a subpath like AA/AABB/AABBCC for a timestamp like AABBCCDDDD.

Example (timestamp is 1362495702) :

  /datadir/hostname/plugin-instance/13/1361/136249/type-instance-1362495702.gz

The I<Plugin> directive can be used (one or more times) to build the include list of notifications to write.

The I<InvertPluginList>, when set, transforms the include list into an exclude list.

The I<LinkLast> directive can be used (one or more times) to build the list of plugins (the same as in I<Plugin>)
for which a symlink will be created and linked to the last notification file.

Example (timestamp is 1362495702) : when our file /datadir/hostname/plugin-instance/13/1361/136249/type-instance-1362495702.gz
is created, the following symlink will be created (or removed/recreated) :

  /datadir/hostname/plugin-instance/last/type-instance-last.gz -> /datadir/hostname/plugin-instance/13/1361/136249/type-instance-1362495702.gz

Example :

 <Plugin notify_file>
   DataDir /var/lib/collectd/notification
   Plugin "sysconfig"
   Plugin "top"
...
Notifications for plugins I<sysconfig> and I<top> will be written to disk. Additionnaly, there will be links to last notifications
of type I<sysconfig> (but not for I<top>).
=
=head2 Plugin C<notify_snmp>

This plugin use the Net-SNMP library to send notifications as B<SNMP> B<Traps>.
...
The plugin's configuration consists of a number of B<OIDs> and B<Target> blocks.

=head3 B<OIDs> blocks

The B<OIDs> block configure the list of object identifiers to send in the
B<SNMP> B<Trap>. There are sent in the order show.
They are identified by the name that is given in the opening line of the block.

=over 4

=item B<EnterpriseOID> I<OID>

Set the Enterprise I<OID>, used in B<SNMP> version 1.

=item B<TrapOID> I<OID>

Set the Trap I<OID>, used in B<SNMP> version 2.

=item B<TimeStampOID> I<OID>

Set the I<OID> for the notification timestamp.

=item B<SeverityOID> I<OID>

Set the I<OID> for the notification severity.

=item B<HostOID> I<OID>

Set the I<OID> for the notification host.

=item B<PluginOID> I<OID>

Set the I<OID> for the notification plugin.

=item B<PluginInstanceOID> I<OID>

Set the I<OID> for the notification plugin instance.

=item B<TypeOID> I<OID>

Set the I<OID> for the notification type.

=item B<TypeInstanceOID> I<OID>

Set the I<OID> for the notification type instance.

=item B<DataSourceOID> I<OID>

Set the I<OID> for the notification data source.

=item B<ValueOID> I<OID>

Set the I<OID> for the notification value of the data source.

=item B<MessageOID> I<OID>

Set the I<OID> for the notification message.

=back

=head3 B<Target> blocks

The B<Target> block configure the destination of the B<SNMP> B<Trap> and
...
the port is 161. See the L<snmpcmd(1)> manual page for more details.

=item B<Version> I<1|2>

Set the SNMP version to use. When giving B<2> version C<2c> is actually used.
Version 3 is not supported by this plugin.

=item B<Community> I<comunity>

Pass the I<Community> to the target.

=item B<OIDs> I<name>

Defines which OIDs are used in the Trap. I<name> refers to one of the B<OIDs>
block above. Since the config file is read top-down you need to define the data
before using it here.

If option B<OIDs> is not specified the default values defined in the plugin
are used.

=item B<SessionReuse> B<true>|B<false>
...
=item B<ProcessMatch> I<name> I<regex> I<ip> I<port>

Gather statistics for processes matching the specified I<regex> for the network
socket opened on I<ip>:I<port>.

The statistics are Recv and Transmit queues, as seen with C<netstat -natp | egrep regex>

I<name> is the name (or prefix) of the metric. It should not contain slashes. It should
be empty.
I<regex> is a regex to match some processes (see L<regex(7)> for details). If empty,
statistics are on all processes opening a socket on I<ip>:I<port>.
I<ip> is a string in the form x.x.x.x for IPv4 addresses or in the form x:x:x:x:x:x:x:x
for IPv6 addresses (see L<inet_pton(3)> for more information). If empty, statistics are 
on any IP the matched processes work with.
I<port> is a number to match a port number. If nul, statistics are on any port the
matched processes work with.
...
=item B<RRA> I<rra types or rra types list>

Specify the RRA to create. If absent, AVERAGE, MIN and MAX will be created.
You can either give a sublist of RRA to create (eg "AVERAGE MIN") or multiple
lines with RRA sublists. Examples :
...
=item B<RRADef> I<timespan> I<pdp per row> I<precision> I<rra types list> I<XFF>

Fully specify the RRA to create. This will define a set of 1 to 3 RRA (according
to the types list you specify) with the given timespan, pdp per row and XFF.

This will overwrite the definition of RRATimespan if given.

I<timespan> : same as RRATimespan

I<pdp per row> : defines the precision of a RRA (multiply with the interval to know it).
0 means default.

I<precision> : If I<pdp per row> is 0 and I<precision> is not 0, I<pdp per row> 
will be computed : I<pdp per row> = max(I<precision> / Interval , 1) where the Interval is
either forced with B<StepSize> or the interval of the metric.

I<rra types list> : same as RRA. "default" means default (either from B<RRA> or the built-in
default MIN MAX AVERAGE)

I<XFF> : same as XFF. Not in range value means default (either from B<XFF> or
the built-in default 0.1).

When receiving metrics with interval=60, configuration is computed as it was :
RRADef     3600   1 0 MIN MAX AVERAGE 0.1
RRADef    86400   1 0 MIN MAX AVERAGE 0.1
RRADef   604800   8 0 MIN MAX AVERAGE 0.1
RRADef  2678400  37 0 MIN MAX AVERAGE 0.1
RRADef 31622400 439 0 MIN MAX AVERAGE 0.1

Note : when interval is different, the pdp per row may change if you do not use RRADef.
This is why you may want to use RRADef instead of only RRATimespan.

Another example : 1 set of RRA with min, max and average for 1 day with the best precision,
then 1 RRA with only average for 1 year with a precision as 1 minute if possible. XFF is
not specified so we get the default value.

RRADef    86400 1  0 MIN MAX AVERAGE
...
=head2 Plugin C<write_top>

The C<write_top> plugin selects I<top> notifications and write them to disk.

Synopsis:

 <Plugin write_top>
     DataDir 
     FlushWhenOlderThanMin 60
...
Set the directory to store Top-files under. Per default RRD-files are generated
beneath the daemon's working directory, i.E<nbsp>e. the B<BaseDir>.

=item B<FlushWhenOlderThanMin> I<Duration>

When set, I<Duration> is the time (in minutes) after when all the top data
will be flushed on disk for one host.

=item B<FlushWhenBiggerThanK> I<Size>

When set, I<Size> is the maximum size (in Kb) of memory used for the cache of one host
for top data. When more memory is needed for one host, the data will be flushed.
